(function() {
	...
	function i18n(ele, options) {
		...
		_checkURLContent(obj.url)
			.then(hasContent => {
				if (hasContent) {
					console.log('i18n_' + options.lang,'Loaded successfully');
					i18nLang = _getJSON(obj);
				} else {
					console.log('i18n_' + options.lang,'failed to load');
					obj.url = options.filePath + options.filePrefix + options.defaultLang + options.fileSuffix + ".json";
					i18nLang = _getJSON(obj);
				}
			});
