...
(function (Scratch) {
    ...
    class rxFS {
        ...
        searchin({ STR }) {
            var Search = [];
            var i;
            for (var i_index in rxFSfi) {
                i = rxFSfi[i_index];
                if (i.indexOf(STR) + 1 != 0) {
                    Search.push(rxFSsy[i_index]);
                }
            }
        ...
        in({ STR }) {
            rxFSfi = JSON.parse(decodeURIComponent(STR.slice(0, (STR.indexOf('|') + 1) - 1)));
            rxFSsy = JSON.parse(decodeURIComponent(STR.slice(((STR.indexOf('|') + 1 + 1) - 1), STR.length)));
        }

        out() {
            return encodeURIComponent(JSON.stringify(rxFSfi)) + '|' + encodeURIComponent(JSON.stringify(rxFSsy));
        }
    }
